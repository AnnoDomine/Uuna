...
@pytest.fixture
def mock_db():
    with patch("Tools.toolsets.tools.audit.evaluate_agent_output.db") as mocked_db:
        yield mocked_db
...
def test_evaluate_agent_output_blind_scoring(mock_db, mock_ask_ai):
    # Setup:
    # 1. DB returns max_potential = 200 for this event
    mock_db.execute.return_value.fetchone.return_value = (200,)
    ...
    result = evaluate_agent_output(mock_ask_ai, "task-123", "event-456", agent_output, reported_confidence)
...
def test_evaluate_agent_output_low_confidence_bonus(mock_db, mock_ask_ai):
    ...
    result = evaluate_agent_output(mock_ask_ai, "t", "e", {}, 0.5)
