    ...
    for root, dirs, files in os.walk(QUERY_DIR):
        for file in files:
            ...
            if "f'" in content or "f\"" in content:
                print(f"❌ SECURITY ERROR: Python f-strings detected in SQL file {file_path}. Use '?' placeholders instead.")
                errors += 1
            ...
            for p in placeholders:
                if p not in ALLOWED_FORMAT_KEYS:
                    print(f"❌ VALIDATION ERROR: Unknown placeholder '{{{p}}}' in {file_path}. Allowed: {ALLOWED_FORMAT_KEYS}")
                    errors += 1
