...
class LibrarianResponse(BaseModel):
    knowledge: str = Field(
        ..., description="The summarized knowledge found in the internal base, or 'None' if nothing relevant exists."
    )
    confidence: int = Field(
        ..., description="Confidence score from 0 to 100 regarding the quality of the information.", ge=0, le=100
    )
    needs_more_research: bool = Field(
        ..., description="True if the internal knowledge is insufficient and a deep archive search is required."
    )
    translated_request: str = Field(..., description="The user prompts translated in english.")
    request_builds: list[str] = Field(..., description="List of all builds requested by the user.")
...
class EventEvaluation(BaseModel):
    ...
    honesty_rating: str = Field(
        ..., description="Rating of the agent's honesty regarding their confidence (e.g., 'Excellent', 'Fair', 'Poor')."
    )
...
class ObserverVerdict(BaseModel):
    evaluations: List[EventEvaluation] = Field(
        ..., description="Detailed evaluation for each event in the task history."
    )
    audit_status: str = Field(
        ..., description="Overall quality audit result (e.g., 'EXCELLENT', 'GOOD', 'FAIR', 'POOR')."
    )
...
class AgentLesson(BaseModel):
    ...
    lesson: str = Field(
        ...,
        description="A concise, factual statement of what was learned or improved (e.g., 'Table X column Y refers to Z').",
    )
...
class EventPotential(BaseModel):
    ...
    potential_score: int = Field(
        ..., description="The calculated potential score (difficulty) for this event.", ge=0, le=1000
    )
...
class SecurityAuditStatus(BaseModel):
    ...
    security_verdict: str = Field(
        ..., description="The final security assessment (e.g., 'CLEAN', 'SENSITIVE_DATA_REDACTED', 'FLAGGED')."
    )
