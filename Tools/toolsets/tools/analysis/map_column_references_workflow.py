...
def map_column_references_workflow(db_client: DBClient, build_version: str, auto_import_previous: bool = True) -> Dict[str, Any]:
    ...
    # --- Main Logic ---
    all_tables_res = db_client.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'archive'").fetchall()
    ...
    for table in all_tables:
        ...
        for col in columns:
            if ';' in col or col.lower() in ('id', 'build_id', '_row_hash'): continue
            ...
            if potential_target_base:
                match = _resolve_table_name_headless(
                    potential_target_base, all_tables, user_mappings, 
                    global_mappings, table, col, pending_questions
                )
                if match:
                    ...
                    newly_confirmed_references[table].append({"column": col, "target_table": match, "active_entries": val_count})
