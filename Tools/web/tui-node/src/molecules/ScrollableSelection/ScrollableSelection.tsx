...
type Item<IM extends DefaultMeta = DefaultMeta> = Record<
	"label" | "value" | "id",
	string
> & {
	meta: IM;
...
const ScrollableSelection = <IM extends DefaultMeta = DefaultMeta>({
	...
	const parsePrefixSuffix = useCallback(
		(type: "prefix" | "suffix", value: string, selected = false) => {
			...
			switch (type) {
				case "prefix":
					parser = selected
						? parsers.parseSelectedPrefix
						: parsers.parseUnselectedPrefix;
					break;
				case "suffix":
					parser = selected
						? parsers.parseSelectedSuffix
						: parsers.parseUnselectedSuffix;
					break;
				default:
					return value;
			}
	...
	return (
		<Box
			...
			borderStyle="round"
			borderColor={
				isFocused ? constants.focused_color : constants.un_focused_color
			}
			width={usedOptions.width}
			height={usedOptions.height}
		>
			<Box width={usedOptions.width}>
				<Spacer />
