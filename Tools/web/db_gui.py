...
async def index(request: Request, db: Optional[str] = None, table: Optional[str] = None, search: Optional[str] = None):
    ...
    if not dbs:
        return templates.TemplateResponse("base.html", {"request": request, "ui_theme": ui_theme, "content": "<h2>No databases found. Run a sync first!</h2>"})
    ...
    context = {
        "request": request, "dbs": dbs, "current_db": current_db, "tables": tables,
        "current_table": current_table, "columns": columns, "rows": rows,
        "search_val": search_val, "ref_map": table_refs, "ui_theme": ui_theme
    }
...
async def builds(request: Request):
    ...
    if os.path.exists(db_reg):
        try:
            conn = sqlite3.connect(db_reg); conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT version, is_downloaded, last_synced FROM builds WHERE product='wow' ORDER BY id DESC LIMIT 50")
            builds_info = [dict(row) for row in cursor.fetchall()]
            conn.close()
        except Exception as e: print(f"Registry error: {e}")

    return templates.TemplateResponse("builds.html", {
        "request": request, 
...
async def compare(request: Request, old: Optional[str] = None, new: Optional[str] = None):
    ...
    context = {"request": request, "downloaded": downloaded, "old_ver": old, "new_ver": new, "results": results, "ui_theme": ui_theme}
...
async def sync(request: Request, build: str = Form(...)):
    ...
    if is_syncing:
        return HTMLResponse(content=f'<div class="alert alert-warning">Sync already in progress for {current_sync_build}</div>', status_code=409)
    ...
    async def event_generator():
        ...
        while True:
            ...
            if last_idx < len(sync_logs): 
                for i in range(last_idx, len(sync_logs)): yield f"data: {sync_logs[i]}\n\n"
...
async def settings_get(request: Request):
    conn = sqlite3.connect('Data/dbs/Settings.db'); conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM settings ORDER BY setting_group, name")
    rows = [dict(row) for row in cursor.fetchall()]
    conn.close()
    grouped = {}
    for row in rows: grouped.setdefault(row['setting_group'], []).append(row)
    ui_theme = get_setting('ui_theme', 'dark')
    return templates.TemplateResponse("settings.html", {"request": request, "grouped_settings": grouped, "ui_theme": ui_theme})

@app.post("/settings")
async def settings_post(request: Request):
    form_data = await request.form()
    conn = sqlite3.connect('Data/dbs/Settings.db'); cursor = conn.cursor()
    ...
    for key, s_type in db_settings:
        ...
        if new_val is not None: cursor.execute("UPDATE settings SET value = ? WHERE key = ?", (str(new_val), key))
