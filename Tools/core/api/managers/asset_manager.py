...
class AssetManager:
    ...
    def ingest_asset(self, file_path, file_id, build_version, description=None):
        ...
        search_text = description or f"WoW Asset FileID {file_id}, Extension {ext}, Size {size} bytes. Build: {build_version}"
    ...
    def get_asset(self, content_hash):
        ...
        with duckdb.connect(self.master_db) as conn:
            res = conn.execute("SELECT binary_data FROM archive.assets WHERE content_hash = ?", [content_hash]).fetchone()
