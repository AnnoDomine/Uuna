...
def select_build_interactively(df, prompt_text):
    ...
    while True:
        ...
        if choice == 'n' and current_page < total_pages - 1: current_page += 1
        elif choice == 'p' and current_page > 0: current_page -= 1
        elif choice == 'q': sys.exit(0)
        elif choice.isdigit():
            idx = int(choice) - 1
            if 0 <= idx < len(df): return df.iloc[idx]['version']
...
def cleanup_local_db(version, run_info):
    ...
    except Exception as e: log.error(f"Cleanup Error: {e}")
...
def process_table_local(con, table_name, sqlite_path, build_id, run_info, build_ver):
    ...
    try:
        ...
        con.execute(f"CREATE OR REPLACE TEMP VIEW source_view AS SELECT * FROM sqlite_scan('{sqlite_path}', '{table_name}')")
        ...
        for col in source_cols:
            if col not in existing_cols: con.execute(f"ALTER TABLE {archive_table} ADD COLUMN \"{col}\" VARCHAR")
        
        hash_cols = [f"COALESCE(CAST(\"{c}\" AS VARCHAR), '')" for c in source_cols if c not in ('build_id', '_row_hash')]
        hash_sql = " || ".join(hash_cols)
        common_cols = [f"\"{c}\"" for c in source_cols if c not in ('_row_hash')]
        col_list_str = ", ".join(common_cols) + ", _row_hash"
        
        con.execute(f"INSERT INTO {archive_table} ({col_list_str}) SELECT {', '.join(common_cols)}, md5({hash_sql}) as _row_hash FROM source_view WHERE md5({hash_sql}) NOT IN (SELECT _row_hash FROM {archive_table})")
        con.execute(f"INSERT OR IGNORE INTO archive.build_data_map (build_id, table_name, row_hash) SELECT {build_id}, '{table_name}', md5({hash_sql}) FROM source_view")
        return True
...
def sync_build(row, all_builds, current_count, total_count, auto_migrate=False):
    version = row['version']; build_id = row['id']
    ...
    if os.path.exists(sqlite_path):
        ...
        if size_kb >= 24000 or auto_migrate:
            ...
            if input(f"  [?] Use {version} anyway? [y/N]: ").lower() == 'y': use_local = True
    
    con = get_con(); con.execute("INSTALL sqlite; LOAD sqlite;")
    if use_local:
        lite_con = sqlite3.connect(sqlite_path)
        tables = [r[0] for r in lite_con.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall() if r[0] not in ('builds', 'sqlite_sequence')]
        lite_con.close()
        ...
        if done == len(tables) and len(tables) > 0: cleanup_local_db(version, run_info)
    else:
...
def main():
    ...
    parser.add_argument("--start"); parser.add_argument("--end"); parser.add_argument("--auto-migrate", action="store_true")
